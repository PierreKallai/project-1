...
Location: API/routes/user_identity/recover_password.py (Línea ~211)

Description: El código OTP de recuperación es incorrecto.

Casuistry: Error al teclear el código recibido por email.

ImportantData: ['confirmation_code']

Status: 400

Name: LimitExceededException

Location: API/routes/user_identity/recover_password.py (Línea ~193)

Description: Excedido el límite de intentos de envío de códigos.

Casuistry: Spam o demasiados reintentos en poco tiempo.
...
Status: 500 (Potencialmente 402/429)
...
Casuistry: Error en API de Audio de OpenAI (Timeout/Quota).
...
stripe.error.CardError (Status 402): Tarjeta rechazada o sin fondos.

stripe.error.RateLimitError (Status 429): Demasiadas peticiones a la pasarela.

stripe.error.InvalidRequestError (Status 400): Datos mal formados enviados a Stripe.

Archivo: API/routes/onoratoFarm/get_voice.py (Línea ~335)
Se recomienda capturar específicamente:

openai.APITimeoutError (Status 504): La API tarda en responder.

openai.RateLimitError (Status 429): Cuota de OpenAI excedida.

Archivo: API/routes/user_identity/login.py (Global try/except)
Se recomienda añadir:

client.exceptions.TooManyRequestsException (Status 429): Bloqueo de seguridad AWS por intentos masivos.

client.exceptions.PasswordResetRequiredException (Status 405): Flujo de reseteo obligatorio.

Archivo: API/functions/buckets.py (Línea ~112)
Se recomienda añadir:

botocore.exceptions.EndpointConnectionError (Status 503): Fallo de red al conectar con S3.
